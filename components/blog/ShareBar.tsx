...
export function ShareBar({ shareUrl, title }: Props) {
  ...
  const iconClass =
    'flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-[#3D1F10] transition hover:bg-[#F5F0E6] hover:text-[#8B3A1E]';

  return (
    <>
      <div
        className="fixed left-4 top-1/2 z-40 hidden -translate-y-1/2 flex-col gap-1 rounded-full border border-[#EDE4D3] bg-white p-2 shadow-sm lg:flex"
        role="navigation"
      ...
      >
        ...
        <button type="button" onClick={onCopy} className={iconClass} aria-label={t('copyLink')}>
      ...
      <div
        className={`pointer-events-none fixed bottom-24 left-1/2 z-[110] -translate-x-1/2 rounded-full bg-[#3D1F10] px-4 py-2 text-sm text-[#F5F0E6] shadow-lg transition-opacity duration-300 lg:bottom-8 ${
